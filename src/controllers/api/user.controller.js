...
async function rateReview(request, response) {
    ...
    try {
        ...
        await Book.findByIdAndUpdate(id, { vote: (total / reactions.length).toFixed(2) });
...
async function postEditCommentReview(request, response) {
...
async function rateBlog(request, response) {
    ...
    try {
        ...
        await Blog.findByIdAndUpdate(id, { vote: (total / reactions.length).toFixed(2) });
